# PREHEAT Retreat
## Authored by: Catalina Cobos-Uribe
## Oct 21, 2022
## Group 1 - Neutrophils
## Aim: Identify neutrophil response-associated proteins pre- and post-woodsmoke exposure (lines 296-437)
...
# Filter for significance in PRE

results_matrix_order2 = results_matrix[order(results_matrix$Pval),]
head(results_matrix_order2)

pre.significant <- results_matrix_order2 %>% 
  filter(Pval < 0.05)

# Now with post-exposure

long.data.post = combined.data %>% filter(PrevPost == "Post") %>% 
  dplyr::select(c("Subject_Number", proteomics.data$Gene_Name, "Responder_Status", "Sex", "Race", "Age", "BMI", "Asthma_Status")) %>%
  pivot_longer(cols = -c(Subject_Number, Responder_Status, Sex, Race, Age, BMI, Asthma_Status), names_to = "Gene", values_to = "Value_Post") %>%
  arrange(Gene)
long.data.pre$Value_Post = as.numeric(long.data$Value_Post)
...
results_matrix_post = matrix(NA, nrow=nrow(proteomics.data), ncol=6)
colnames(results_matrix_post) = c("Statistic", "Pval", "Pval.BH", "Mean.R", "Mean.NR", "Log2FC")
rownames(results_matrix_post) = proteomics.data$Gene_Name

for(i in proteomics.data$Gene_Name) {
  
  resp.data = long.data.post.loop[which(long.data.post.loop$Responder_Status=="R"),i]
  resp.data = as.numeric(resp.data)
  
  non.resp.data = long.data.post.loop[which(long.data.post.loop$Responder_Status=="NR"),i]
  non.resp.data = as.numeric(non.resp.data)
  
  t.test.results = t.test(resp.data, non.resp.data)
  
  results_matrix_post[i, "Statistic"] = t.test.results$statistic
  results_matrix_post[i, "Pval"] = t.test.results$p.value
  
  results_matrix_post[i, "Mean.R"] = mean(resp.data)
  results_matrix_post[i, "Mean.NR"] = mean(non.resp.data)
  results_matrix_post[i, "Log2FC"] = log2(mean(non.resp.data)/mean(resp.data))
  
}

results_matrix_post[,"Pval.BH"] = p.adjust(results_matrix_post[,"Pval"], method="BH")

# Convert to dataframe
results_matrix_post = data.frame(results_matrix_post)

# Filter for significance in POST

results_matrix_order_post = results_matrix_post[order(results_matrix_post$Pval),]
head(results_matrix_order_post)

post.significant <- results_matrix_order_post %>% 
  filter(Pval < 0.05)

# Export dataframes

write.table(pre.significant, file = "pre_pmn_significant.txt", sep = ",", quote = FALSE)
write.table(post.significant, file = "post_pmn_significant.txt", sep = ",", quote = FALSE, row.names = F)
